...
from apis import api,Page,APIError,ApiValueError,APIPermissionError,APIResourceNotFoundErrror
...
def register_user():
    ...
    user.insert()
    # make session cookie:
    cookie=make_signed_cookie(user.id,user.passwork,None)
...
def api_get_create():
    ...
    user=ctx.request.user
    blog=Blog(user_id=user.name,user_name=user.name,name=name,summary=summary,content=content)
    blog.insert()
    return blog
...
def api_create_blog_comment(blog_id):
    ...
    c=Comment(blog_id=blog.id,user_id=user.id,user_name=user.name,user_image=user.image,content=content)
    c.insert()
...
def api_get_comments():
    ...
    page=Page(total,_get_page_index())
    comments=Comment.find_by('order by create_at desc limit ?,?',page.offset,page.limit)
...
def api_get_users():
    ...
    page=Page(total,_get_page_index)
    users=User.find_by('order by create_at desc limit ?,?',page.offset,page.limit)
    for u in users:
        u.password='******'

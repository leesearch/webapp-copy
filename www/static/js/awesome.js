...
function showError(err){
    var alter=$('div.uk-alter-danger');
    if(err){
        alter.text(err.message || err.error || err).removeClass('uk-hidden').show();
        try{
            if(alter.offset().top < ($(window).scrollTop()-41)){
                $('html,body').animate({scrollTop : alter.offset().top - 41});
            }
        }
        catch(e) {}
    }
    else{
        alter.addClass('uk-hidden').hide().text('');
    }
}
...
function showConfirm(title, text, fn_ok, fn_cancel){
    var s='<div id="div-confir" class="uk-modal">' +
        '<div class="uk-modal-dialog">' +
        '<a href="#0" class="uk-modal-close uk-close"></a>' +
        '<h1 class="x-title"></h1>' +
        '<p class="x-text"></p>'+
        '<hr><p class="uk-text-center"></p>' +
...
$(function(){
    if(location.pathname ==='/' || location.patname.indexOf('/blog')===0){
